...
const Contact = () => {
 ...
 const handleSuccessClose = (event, reason) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
   if(nameRef.current.value.length > 3 && surnameRef.current.value.length > 3 && emailRef.current.value.length > 3 && messageRef.current.value.length > 3) {emailjs.send('service_zt89lzv', 'template_zo3yfon',  {
  ...
  return (
    <>
      <div className="contact_container">
        ...
        <Snackbar open={success0pen} autoHideDuration={4000} onClose={handleSuccessClose} >
        <Alert onClose={handleSuccessClose} severity="success" sx={{backgroundColor:'rgb(134 203 134)',fontSize:'1rem'}}>
           Správa bola odoslaná!
        </Alert>
      </Snackbar>
      <Snackbar open={dangerOpen} autoHideDuration={4000} onClose={handleDangerClose} >
        <Alert onClose={handleDangerClose} severity="error" sx={{backgroundColor:'rgb(255 129 129)',fontSize:'1rem'}}>
